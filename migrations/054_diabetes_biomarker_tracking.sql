...
CREATE TABLE random_blood_sugar_logs (
    ...
    -- Measurement data
    rbs_mg_dl REAL NOT NULL,
    measured_at TIMESTAMP NOT NULL,
    measurement_method VARCHAR(50),

    -- Meal context (CRITICAL for interpretation)
    hours_since_meal REAL,  -- Time since last meal (key variable)
    meal_id INTEGER,  -- Link to what they ate (enables correlation analysis)
    meal_carbs_g REAL,  -- Carbs in that meal (if available)
    meal_glycemic_load REAL,  -- GL of that meal (if calculated)
...
CREATE TABLE hba1c_logs (
    ...
    -- Measurement data
    hba1c_percent REAL NOT NULL,
    measured_at TIMESTAMP NOT NULL,
    lab_facility_id INTEGER,

    -- Derived metrics
    estimated_avg_glucose_mg_dl REAL,  -- eAG formula: (HbA1c × 28.7) - 46.7
    ...
    previous_hba1c REAL,  -- Previous reading for comparison
    previous_test_date DATE,  -- When was previous test?
    change_from_previous REAL,  -- Positive = worsening, Negative = improving
