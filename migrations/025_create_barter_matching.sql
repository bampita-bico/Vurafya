...
CREATE TABLE IF NOT EXISTS barter_match_scores (
  ...
  match_score REAL NOT NULL, -- 0.0 = terrible match, 1.0 = perfect match
  category_alignment_score REAL, -- 0.0-1.0: Does User A want what User B has?
  value_alignment_score REAL, -- 0.0-1.0: Are values similar?
  location_proximity_score REAL, -- 0.0-1.0: How close geographically?
  trust_score_avg REAL, -- 0.0-1.0: Average of both users' trust scores

  -- Match Details
  value_difference_ap INTEGER, -- Absolute difference in value
  distance_km REAL, -- Calculated distance between users
  ...
  recalculate_after TIMESTAMP DEFAULT (datetime('now', '+1 day')), -- Recalculate daily
...
CREATE TABLE IF NOT EXISTS match_score_weights (
  ...
  weight_value REAL NOT NULL CHECK (weight_value >= 0 AND weight_value <= 1), -- 0.0 to 1.0
  description TEXT,
  is_active BOOLEAN DEFAULT TRUE,
...
INSERT OR IGNORE INTO match_score_weights (weight_name, weight_value, description) VALUES
...
BEGIN
  INSERT OR IGNORE INTO barter_match_notifications (user_id, match_score_id, notification_type, notification_text)
  VALUES (
    NEW.user_a_id,
    NEW.id,
    CASE
