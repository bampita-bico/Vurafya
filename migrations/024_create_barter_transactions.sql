...
CREATE TABLE IF NOT EXISTS barter_exchange_transactions (
  ...
  transaction_fee_pct REAL DEFAULT 3.5, -- 3.5% fee on barter exchanges
  transaction_fee_ap INTEGER, -- Fee charged in Afya Points
  transaction_fee_ugx REAL, -- Converted fee to UGX for accounting
...
BEGIN
  -- Lock Party A's item
  INSERT OR IGNORE INTO barter_escrow_locks (transaction_id, item_type, item_id, locked_by_user_id, expires_at)
  VALUES (NEW.id, NEW.party_a_offer_type, NEW.party_a_offer_id, NEW.party_a_user_id, datetime('now', '+48 hours'));

  -- Lock Party B's item (if it's a barter item, not a healthcare service)
  INSERT OR IGNORE INTO barter_escrow_locks (transaction_id, item_type, item_id, locked_by_user_id, expires_at)
  SELECT NEW.id, NEW.party_b_offer_type, NEW.party_b_offer_id, NEW.party_b_user_id, datetime('now', '+48 hours')
...
BEGIN
  INSERT OR IGNORE INTO barter_transaction_timeline (transaction_id, status_from, status_to, automated)
