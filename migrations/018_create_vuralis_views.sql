...
LEFT JOIN (
  SELECT user_id,
    AVG(CAST(json_extract(data_json, '$.hours') AS REAL)) AS avg_sleep_hours
  FROM (
...
LEFT JOIN (
  SELECT user_id,
    AVG(CAST(json_extract(nutrition_json, '$.protein_g') AS REAL)) AS avg_protein_g,
    AVG(CAST(json_extract(nutrition_json, '$.fiber_g') AS REAL)) AS avg_fiber_g,
