...
CREATE TABLE IF NOT EXISTS user_kyc_status (
  ...
  max_transaction_ugx REAL, -- NULL = unlimited for Tier 3
...
CREATE TABLE IF NOT EXISTS aml_screening_log (
  ...
  -- Risk Indicators
  risk_score REAL NOT NULL, -- 0.0 to 1.0 (0 = no risk, 1 = high risk)
  risk_level VARCHAR(40), -- low / medium / high / critical
...
CREATE TABLE IF NOT EXISTS tax_reporting_log (
  ...
  barter_earnings_ugx REAL DEFAULT 0, -- Value received in barter
  platform_commission_earnings_ugx REAL DEFAULT 0, -- For facility/pharmacy partners

  -- Tax Thresholds
  tax_threshold_ugx REAL, -- Reporting threshold for this country
...
CREATE TABLE IF NOT EXISTS country_regulatory_requirements (
  ...
  kyc_threshold_ugx REAL, -- Transaction value requiring KYC
  kyc_renewal_months INTEGER DEFAULT 24, -- How often KYC needs renewal

  -- AML Requirements
  aml_screening_required BOOLEAN DEFAULT TRUE,
  aml_threshold_ugx REAL, -- Transaction value requiring AML screening
  suspicious_transaction_threshold_ugx REAL, -- Auto-flag transactions above this

  -- Tax Reporting
  tax_reporting_required BOOLEAN DEFAULT TRUE,
  tax_reporting_threshold_ugx REAL, -- Annual earnings threshold for reporting
...
INSERT OR IGNORE INTO country_regulatory_requirements (
  country_code,
  kyc_threshold_ugx,
  aml_threshold_ugx,
...
FOR EACH ROW
BEGIN
  INSERT OR IGNORE INTO user_kyc_status (user_id, kyc_level, kyc_tier, max_transaction_ugx)
  VALUES (NEW.id, 'none', 0, 50000); -- New users limited to 50K UGX without KYC

  INSERT OR IGNORE INTO data_privacy_consent (user_id)
  VALUES (NEW.id);
END;
...
BEGIN
  INSERT OR IGNORE INTO regulatory_compliance_log (
    compliance_type,
    user_id,
    transaction_id,
    transaction_type,
