...
CREATE TABLE IF NOT EXISTS virtual_currency_config (
    ...
    peg_rate REAL NOT NULL DEFAULT 1.0,
        -- 1 VRC = X of peg currency
    min_transaction REAL DEFAULT 1.0,
    max_transaction REAL DEFAULT 999999999.0,
...
CREATE TABLE IF NOT EXISTS vrc_transactions (
    ...
    conversion_fee_vrc REAL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS cross_platform_currency_bridge (
    ...
    points_to_vrc_rate REAL NOT NULL,
        -- How many points = 1 VRC
    vrc_to_points_rate REAL NOT NULL,
        -- 1 VRC = how many points
    conversion_fee_pct REAL DEFAULT 2.0,
        -- Fee for converting between points and VRC
    min_conversion_amount REAL DEFAULT 10.0,
    max_daily_conversion REAL DEFAULT 100000.0,
    is_active BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS points_vrc_conversions (
    ...
    vrc_amount REAL NOT NULL,
    conversion_rate REAL NOT NULL,
    fee_vrc REAL DEFAULT 0,
...
INSERT OR IGNORE INTO virtual_currency_config (id, currency_name, currency_code, currency_symbol, description, peg_type, peg_currency_code, peg_rate, decimal_places, applies_to_platforms)
...
INSERT OR IGNORE INTO virtual_currency_basket (virtual_currency_id, component_currency_code, weight_pct, rationale, is_active) VALUES
...
INSERT OR IGNORE INTO virtual_currency_rate_history (currency_code, rate_to_ugx, peg_type, change_reason)
...
INSERT OR IGNORE INTO cross_platform_currency_bridge (platform_name, points_currency_code, points_currency_name, points_to_vrc_rate, vrc_to_points_rate, conversion_fee_pct) VALUES
...
INSERT OR IGNORE INTO universal_currency_matrix (currency_type, currency_code, ugx_exchange_rate, is_base_currency, last_updated, is_active)
