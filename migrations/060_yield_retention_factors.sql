...
CREATE TABLE cooking_methods (
    ...
    vitamin_retention_avg REAL DEFAULT 1.0,  -- 0.0-1.0 (1.0 = 100% retained)
    mineral_retention_avg REAL DEFAULT 1.0,
    protein_impact REAL DEFAULT 1.0,  -- Usually stable
    fat_impact REAL DEFAULT 1.0,  -- Can increase with frying (>1.0)
    carb_impact REAL DEFAULT 1.0,
    fiber_impact REAL DEFAULT 1.0,

    -- Physical changes
    water_content_change REAL DEFAULT 0.0,  -- Negative = loss (evaporation), Positive = gain (absorption)
    weight_yield_factor REAL DEFAULT 1.0,  -- Cooked weight / raw weight (0.9 = 10% weight loss)
    volume_change REAL DEFAULT 1.0,  -- Cooked volume / raw volume
...
INSERT OR IGNORE INTO cooking_methods VALUES
...
CREATE TABLE food_cooking_method_retention (
    ...
    protein_retention REAL DEFAULT 1.0,
    carb_retention REAL DEFAULT 1.0,
    fat_retention REAL DEFAULT 1.0,
    fiber_retention REAL DEFAULT 1.0,

    -- Vitamin-specific retention (water-soluble vitamins most affected by boiling)
    vitamin_c_retention REAL,  -- Most vulnerable (oxidation + water leaching)
    vitamin_b1_retention REAL,  -- Thiamine (heat-sensitive)
    vitamin_b2_retention REAL,  -- Riboflavin
    vitamin_b3_retention REAL,  -- Niacin (most stable)
    vitamin_b6_retention REAL,  -- Pyridoxine
    vitamin_b9_retention REAL,  -- Folate (very heat-sensitive)
    vitamin_b12_retention REAL,  -- Cobalamin (stable in cooking)
    vitamin_a_retention REAL,  -- Fat-soluble (stable, can increase bioavailability with heat)
    vitamin_d_retention REAL,
    vitamin_e_retention REAL,
    vitamin_k_retention REAL,

    -- Mineral retention (generally stable but water-soluble)
    potassium_retention REAL,  -- Leaches into cooking water
    sodium_retention REAL,  -- Usually stable (can increase if salt added)
    calcium_retention REAL,
    iron_retention REAL,
    magnesium_retention REAL,
    phosphorus_retention REAL,
    zinc_retention REAL,

    -- Physical changes
    weight_yield REAL,  -- Cooked weight = raw weight × yield
    volume_yield REAL,
...
INSERT OR IGNORE INTO food_cooking_method_retention VALUES
