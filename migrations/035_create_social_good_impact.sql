...
CREATE TABLE IF NOT EXISTS social_good_impact_log (
  ...
  -- Impact Metrics
  transaction_value_ugx REAL NOT NULL, -- Full value of goods/services provided
  fee_waived_ugx REAL NOT NULL, -- Platform revenue sacrificed for social good
  fee_waived_pct REAL, -- % of transaction value waived
...
CREATE TABLE IF NOT EXISTS monthly_social_good_summary (
  ...
  -- Value Delivered
  total_value_delivered_ugx REAL DEFAULT 0, -- Total goods/services provided
  total_fees_waived_ugx REAL DEFAULT 0, -- Revenue sacrificed
...
INSERT OR IGNORE INTO social_good_goals (goal_period, target_users_served, target_value_delivered_ugx, target_essential_medicines, target_rural_communities, target_zero_cash_transactions) VALUES
...
BEGIN
  INSERT OR IGNORE INTO social_good_impact_log (
    transaction_id,
    transaction_type,
    user_id,
    user_category,
...
FOR EACH ROW
BEGIN
  INSERT OR IGNORE INTO monthly_social_good_summary (
    month,
    total_users_served,
...
BEGIN
  INSERT OR IGNORE INTO vulnerable_population_registry (user_id, is_elderly, verification_method)
