...
CREATE TABLE IF NOT EXISTS labor_currency_conversion (
  ...
  base_hourly_rate_ugx REAL NOT NULL, -- In UGX (for display)
  base_hourly_rate_local_currency REAL, -- In local currency (NGN, KES, etc.)

  -- Multipliers
  proficiency_multiplier REAL NOT NULL, -- beginner: 0.8, intermediate: 1.0, expert: 1.5
  country_cost_of_living_multiplier REAL NOT NULL, -- Nigeria: 1.2, Uganda: 1.0, Ethiopia: 0.8
  demand_surge_multiplier REAL DEFAULT 1.0, -- High-demand skills: 1.3
  ...
  minimum_wage_local_currency REAL, -- Country's minimum wage per hour
...
CREATE TABLE IF NOT EXISTS country_cost_of_living (
  ...
  cost_of_living_index REAL NOT NULL, -- Base 100 = Uganda, Nigeria 120, Ethiopia 80
  minimum_wage_hourly_local_currency REAL, -- Official minimum wage per hour
  minimum_wage_hourly_ugx REAL, -- Converted to UGX
  currency_strength_index REAL, -- Purchasing power (base 1.0 = UGX)
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (country_code) REFERENCES countries_supported(country_code)
);

-- Seed cost of living data for key African countries
INSERT OR IGNORE INTO country_cost_of_living (country_code, cost_of_living_index, minimum_wage_hourly_local_currency, minimum_wage_hourly_ugx, currency_strength_index) VALUES
-- East Africa
...
INSERT OR IGNORE INTO proficiency_level_definitions (proficiency_level, multiplier, years_experience_min, years_experience_max, description) VALUES
...
CREATE TABLE IF NOT EXISTS labor_demand_surge (
  ...
  surge_multiplier REAL NOT NULL, -- 0.8 (low demand) to 1.5 (high demand)
  active_requests INTEGER, -- Number of unfulfilled labor requests
  active_providers INTEGER, -- Number of available providers
  supply_demand_ratio REAL, -- active_providers / active_requests
  calculated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  expires_at TIMESTAMP DEFAULT (datetime('now', '+7 days')), -- Recalculate weekly
  FOREIGN KEY (skill_category_id) REFERENCES labor_skill_categories(id) ON DELETE CASCADE
);
...
INSERT OR IGNORE INTO labor_currency_conversion (skill_category_id, country_code, proficiency_level, base_hourly_rate_ap, base_hourly_rate_ugx, proficiency_multiplier, country_cost_of_living_multiplier, final_hourly_rate_ap, final_hourly_rate_ugx)
SELECT
  lsc.id,
  'UG',
