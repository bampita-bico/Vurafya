...
CREATE TABLE meal_nutrition_calculations (
    ...
    weight_yield_factor DOUBLE PRECISION,  -- cooked / raw
    average_retention_factor DOUBLE PRECISION,  -- Average of all nutrients

    -- === CLINICAL METRICS ===
    glycemic_load DOUBLE PRECISION,
    glycemic_load_category VARCHAR(20),  -- low / medium / high
    pral_value DOUBLE PRECISION,  -- Potential Renal Acid Load (for CKD)
    pral_category VARCHAR(20),  -- alkaline / neutral / acidic
    sodium_potassium_ratio DOUBLE PRECISION,  -- Important for hypertension

    -- === DATA QUALITY ===
    calculation_warnings TEXT,  -- JSON array of warnings
    data_quality_score DOUBLE PRECISION,  -- 0.0-1.0 (1.0 = all measured data, <0.5 = heavily estimated)
...
CREATE TABLE nutrition_calculation_comparisons (
    ...
    -- Comparison
    calories_difference_pct DOUBLE PRECISION,  -- (calculated - user_provided) / user_provided × 100
