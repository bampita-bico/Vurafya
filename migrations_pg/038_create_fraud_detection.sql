...
CREATE TABLE IF NOT EXISTS fraud_detection_alerts (
  ...
  -- Risk Score
  risk_score DOUBLE PRECISION NOT NULL, -- 0.0 to 1.0 (0 = no risk, 1 = certain fraud)
...
CREATE TABLE IF NOT EXISTS fraud_detection_rules (
  ...
  -- Thresholds
  risk_score_weight DOUBLE PRECISION DEFAULT 0.1, -- How much this rule contributes to overall risk score (0.0-1.0)
  alert_threshold DOUBLE PRECISION DEFAULT 0.7, -- Trigger alert if risk_score >= this threshold
...
INSERT INTO fraud_detection_rules (rule_code, rule_name, rule_category, rule_condition, risk_score_weight, alert_threshold, auto_action, description) VALUES
...
CREATE TABLE IF NOT EXISTS user_fraud_history (
  ...
  -- Risk Profile
  overall_fraud_risk_score DOUBLE PRECISION DEFAULT 0.0, -- 0.0 to 1.0
...
FOR EACH ROW
BEGIN
  INSERT INTO user_fraud_history (user_id)
  VALUES (NEW.id);
END;
...
1. DOUBLE PRECISION-TIME TRANSACTION MONITORING
