...
CREATE TABLE IF NOT EXISTS unified_payment_ledger (
  ...
  total_amount_ugx DOUBLE PRECISION NOT NULL, -- Total transaction value in UGX
  total_amount_local_currency DOUBLE PRECISION, -- In payer's local currency
  ...
  -- Platform Fees
  transaction_fee_pct DOUBLE PRECISION, -- Variable by transaction type (0-5%)
  ...
  partner_revenue_ugx DOUBLE PRECISION, -- For facility/pharmacy partners
...
CREATE TABLE IF NOT EXISTS hybrid_payment_details (
  ...
  component_amount DOUBLE PRECISION NOT NULL, -- Amount in native unit (UGX, AP, hours, credits)
  component_value_ugx DOUBLE PRECISION NOT NULL, -- Converted to UGX for accounting
  component_percentage DOUBLE PRECISION, -- % of total payment
...
CREATE TABLE IF NOT EXISTS user_payment_preferences (
  ...
  max_labor_hours_per_month DOUBLE PRECISION, -- Don't commit more than X hours per month
...
FOR EACH ROW
BEGIN
  INSERT INTO user_payment_preferences (user_id, default_fiat_currency)
  VALUES (NEW.id, 'UGX');
END;
