-- Migration 031: Create Trust & Verification System
-- Date: 2026-04-11
...
CREATE TABLE IF NOT EXISTS user_trust_ratings (
  ...
  trust_score DOUBLE PRECISION DEFAULT 0.5, -- Start neutral at 0.5
  trust_level VARCHAR(40) DEFAULT 'building', -- building / low / medium / high / verified_trader

  -- Component Scores (0.0 to 1.0)
  barter_reliability DOUBLE PRECISION DEFAULT 0.5, -- % of barter trades completed successfully
  labor_reliability DOUBLE PRECISION DEFAULT 0.5, -- % of labor hours verified without disputes
  payment_reliability DOUBLE PRECISION DEFAULT 0.5, -- % of payments made on time
  verification_rate DOUBLE PRECISION DEFAULT 0.0, -- % of transactions with facility/peer verification
  avg_rating DOUBLE PRECISION DEFAULT 0.0, -- 0.0-5.0 peer review average
  ...
  max_transaction_ugx DOUBLE PRECISION, -- NULL = unlimited (for high trust users)
...
CREATE TABLE IF NOT EXISTS trust_score_weights (
  ...
  weight_value DOUBLE PRECISION NOT NULL, -- 0.0 to 1.0
  description TEXT,
...
INSERT INTO trust_score_weights (weight_name, weight_value, description) VALUES
...
CREATE TABLE IF NOT EXISTS trust_actions_log (
  ...
  trust_score_change DOUBLE PRECISION, -- Positive or negative
...
CREATE INDEX IF NOT EXISTS idx_trust_ratings_user ON user_trust_ratings(user_id);
CREATE INDEX IF NOT EXISTS idx_trust_ratings_score ON user_trust_ratings(trust_score);
CREATE INDEX IF NOT EXISTS idx_trust_ratings_level ON user_trust_ratings(trust_level);
CREATE INDEX IF NOT EXISTS idx_trust_ratings_suspended ON user_trust_ratings(is_suspended);

CREATE INDEX IF NOT EXISTS idx_transaction_reviews_transaction ON transaction_reviews(transaction_id, transaction_type);
CREATE INDEX IF NOT EXISTS idx_transaction_reviews_reviewer ON transaction_reviews(reviewer_user_id);
CREATE INDEX IF NOT EXISTS idx_transaction_reviews_reviewee ON transaction_reviews(reviewed_user_id);
CREATE INDEX IF NOT EXISTS idx_transaction_reviews_rating ON transaction_reviews(rating);

CREATE INDEX IF NOT EXISTS idx_trust_actions_user ON trust_actions_log(user_id);
CREATE INDEX IF NOT EXISTS idx_trust_actions_type ON trust_actions_log(action_type);
CREATE INDEX IF NOT EXISTS idx_trust_actions_created ON trust_actions_log(created_at);

-- View: User Trust Profile (for easy lookup)
CREATE VIEW IF NOT EXISTS v_user_trust_profile AS
SELECT
  u.id AS user_id,
  u.username,
  utr.trust_score,
...
CREATE TRIGGER IF NOT EXISTS trg_create_default_trust_rating
AFTER INSERT ON users
FOR EACH ROW
BEGIN
  INSERT INTO user_trust_ratings (user_id, trust_score, trust_level, max_transaction_ugx)
...
BEGIN
  ...
  INSERT INTO trust_actions_log (user_id, action_type, reference_id, reference_table, action_description)
  VALUES (
...
CREATE TRIGGER IF NOT EXISTS trg_update_trust_after_barter
...
BEGIN
  ...
  SET
    ...
    barter_reliability = CAST(barter_trades_completed AS DOUBLE PRECISION) / NULLIF(barter_trades_initiated, 0),
    updated_at = CURRENT_TIMESTAMP
...
CREATE TRIGGER IF NOT EXISTS trg_penalize_trust_barter_dispute
AFTER UPDATE OF status ON barter_exchange_transactions
FOR EACH ROW
...
BEGIN
  ...
  INSERT INTO trust_actions_log (user_id, action_type, reference_id, reference_table, action_description, penalty_points)
...
CREATE TRIGGER IF NOT EXISTS trg_update_trust_after_labor
...
BEGIN
  ...
  SET
    ...
    completed_transactions = completed_transactions + 1,
    labor_reliability = CAST(labor_hours_verified AS DOUBLE PRECISION) / NULLIF(labor_hours_committed, 0),
    updated_at = CURRENT_TIMESTAMP
...
CREATE TRIGGER IF NOT EXISTS trg_penalize_trust_no_show
...
CREATE TRIGGER IF NOT EXISTS trg_auto_assign_trust_level
...
CREATE TRIGGER IF NOT EXISTS trg_auto_suspend_bad_actors
...
BEGIN
  ...
  SET
    is_suspended = TRUE,
    suspended_until = CURRENT_TIMESTAMP + INTERVAL '30 days',
    suspension_reason = 'Accumulated 100+ penalty points for repeated violations'
  WHERE id = NEW.id;

  INSERT INTO trust_actions_log (user_id, action_type, action_description, penalty_points)
