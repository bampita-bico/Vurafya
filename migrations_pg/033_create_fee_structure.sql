...
CREATE TABLE IF NOT EXISTS fee_structure_config (
  ...
  -- Base Fee Rates
  base_fee_pct DOUBLE PRECISION NOT NULL, -- Base percentage (3.5% for barter, 5% for labor, 2% for verification, 1% for escrow)
  base_fee_ap INTEGER, -- Flat fee in Afya Points (alternative to percentage)
  base_fee_ugx DOUBLE PRECISION, -- Flat fee in UGX (alternative to percentage)

  -- Fee Split (for percentage fees)
  platform_share_pct DOUBLE PRECISION DEFAULT 100.0, -- % of fee that goes to platform
  partner_share_pct DOUBLE PRECISION DEFAULT 0.0, -- % of fee that goes to facility/pharmacy partner
  worker_share_pct DOUBLE PRECISION DEFAULT 0.0, -- % of fee charged to worker (for labor)

  -- Minimum & Maximum Fees
  min_fee_ugx DOUBLE PRECISION, -- Minimum fee (protects revenue on micro-transactions)
  max_fee_ugx DOUBLE PRECISION, -- Maximum fee cap (protects users on large transactions)
...
INSERT INTO fee_structure_config (fee_type, base_fee_pct, applies_to, description, waive_for_essential_medicines, waive_for_vulnerable_populations, waive_for_micro_transactions) VALUES
...
INSERT INTO country_fee_adjustments (country_code, fee_type, adjusted_fee_pct, adjustment_reason) VALUES
...
CREATE TABLE IF NOT EXISTS volume_discount_tiers (
  ...
  -- Discount Rates
  fee_discount_pct DOUBLE PRECISION NOT NULL, -- % discount off base fee (e.g., 10% = 0.9× multiplier)
...
INSERT INTO volume_discount_tiers (tier_name, min_transactions_per_month, max_transactions_per_month, fee_discount_pct, tier_description, tier_badge_icon) VALUES
...
CREATE TABLE IF NOT EXISTS promotional_campaigns (
  ...
  discount_pct DOUBLE PRECISION NOT NULL, -- 0-100 (100 = free)
