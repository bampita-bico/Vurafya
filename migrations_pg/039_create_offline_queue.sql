...
INSERT INTO offline_capability_status (feature_name, supports_offline, offline_mode, requires_immediate_sync, can_batch_sync, conflict_resolution_strategy, description) VALUES
...
SELECT
  ...
  CASE
    WHEN dss.total_sync_count > 0 THEN CAST(dss.success_sync_count AS DOUBLE PRECISION) / dss.total_sync_count * 100
    ELSE 0
...
BEGIN
  ...
  SET
    ...
    next_sync_attempt_at = CASE
      WHEN sync_attempt_count < 3 THEN CURRENT_TIMESTAMP + INTERVAL '5 minutes' -- Retry after 5 min (first 3 attempts)
      WHEN sync_attempt_count < 10 THEN CURRENT_TIMESTAMP + INTERVAL '30 minutes' -- Retry after 30 min (4-10 attempts)
      ELSE CURRENT_TIMESTAMP + INTERVAL '2 hours' -- Retry after 2 hours (11+ attempts)
    END
...
FOR EACH ROW
BEGIN
  INSERT INTO device_sync_state (user_id, device_id, last_sync_at, pending_transactions)
  VALUES (
    NEW.user_id,
    NEW.device_id,
    CURRENT_TIMESTAMP,
