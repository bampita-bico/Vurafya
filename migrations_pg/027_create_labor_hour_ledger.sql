...
CREATE TABLE IF NOT EXISTS labor_hour_ledger (
  ...
  total_earned_ugx DOUBLE PRECISION, -- Converted to UGX for accounting

  -- Work Date & Time
  work_date DATE NOT NULL,
  start_time TIMESTAMP,
  end_time TIMESTAMP,
  actual_duration_hours DOUBLE PRECISION, -- Calculated from start/end time
...
CREATE TABLE IF NOT EXISTS labor_earnings_summary (
  ...
  verified_hours DOUBLE PRECISION DEFAULT 0, -- Hours with verification
...
BEGIN
  INSERT INTO labor_verification_queue (labor_hour_ledger_id, verification_type, priority)
  VALUES (
    NEW.id,
...
WHEN NEW.is_verified = TRUE AND OLD.is_verified = FALSE
BEGIN
  INSERT INTO labor_earnings_summary (user_id, service_type, month, total_hours_worked, total_earned_ap, total_earned_ugx, jobs_completed, verified_hours)
  VALUES (
    NEW.user_id,
    NEW.service_type,
...
BEGIN
  ...
  INSERT INTO afya_points_ledger (user_id, points_change, transaction_type, action_type, reference_id, reference_table, balance_after, notes)
  VALUES (
    NEW.user_id,
    NEW.net_earned_ap,
    'earned',
  ...
  INSERT INTO platform_revenue_ledger (revenue_source, amount_ugx, amount_ap, user_id, reference_id, reference_table)
  VALUES (
