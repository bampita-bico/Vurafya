...
CREATE TABLE IF NOT EXISTS glycemic_load_index (
    ...
    available_carb_g DOUBLE PRECISION,   -- Net carbs (total - fiber)
    glycemic_load DOUBLE PRECISION,      -- GL = (GI × available_carb_g) / 100
...
CREATE TABLE IF NOT EXISTS renal_acid_load_data (
    id SERIAL PRIMARY KEY,
    food_id INTEGER NOT NULL,
    pral_value DOUBLE PRECISION NOT NULL,      -- Potential Renal Acid Load (mEq/100g)
    neap_value DOUBLE PRECISION,                -- Net Endogenous Acid Production
    acid_category VARCHAR(20),      -- acidic / neutral / alkaline
    phosphorus_mg DOUBLE PRECISION,             -- Key mineral for CKD
    potassium_mg DOUBLE PRECISION,              -- Key mineral for CKD
    sodium_mg DOUBLE PRECISION,                 -- Key mineral for CKD
    protein_g DOUBLE PRECISION,                 -- Affects acid load
...
CREATE TABLE IF NOT EXISTS body_composition_log (
    ...
    bmi DOUBLE PRECISION,                      -- Calculated or measured
...
CREATE TABLE IF NOT EXISTS sleep_stages_log (
    ...
    sleep_efficiency_pct DOUBLE PRECISION,     -- (total_sleep / time_in_bed) × 100
    sleep_quality_score DOUBLE PRECISION,      -- 0.0-1.0 or 0-100
    resting_heart_rate INTEGER,
    hrv_avg DOUBLE PRECISION,                  -- Heart rate variability
...
CREATE TABLE IF NOT EXISTS heart_rate_variability (
    ...
    user_id INTEGER NOT NULL,
    recorded_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    hrv_ms DOUBLE PRECISION NOT NULL,          -- Milliseconds (RMSSD standard)
    measurement_type VARCHAR(20),  -- resting / waking / post_exercise
    resting_hr INTEGER,
    recovery_score DOUBLE PRECISION,           -- 0.0-1.0 or 0-100
...
CREATE TABLE IF NOT EXISTS glucose_monitoring (
    ...
    insulin_dose_units DOUBLE PRECISION,         -- If applicable
